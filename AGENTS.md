...
1. **Create** `memory/sessions/YYYY-MM-DD-HHMMSS.md` using the UTC timestamp at
   ...
   **Never write secrets or PII into `memory/`** — session logs and continuity are committed
   ...
   (Field incident, reported 2026-08-13: pasted smoke-test output carried a live OAuth client
   secret into a client repo's committed session log — their DLP scanner caught it.)
