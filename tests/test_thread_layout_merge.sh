...
#   (2) both sides editing the SAME thread file still conflicts — the MERGE.md Tier 2
#       human gate is load-bearing, so a clean merge here would be a regression;
...
printf 'PASS: thread layout merge contract — different-threads clean, same-thread conflicts, archive union (4 cases)\n'
