...
`.gitlab/agent-memory-ci.yml` on GitLab (v4.31.0) — it runs server-side on every push and pull/merge
request with no per-user setup, so the ritual is enforced even on a clone where the local hook was
never activated. (One honest limit: a self-managed GitLab instance needs an admin-registered runner.)
