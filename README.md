...
| 4.22.1 | **post-commit auto-stub is per *session*, not per *commit*:** the hook's auto-stub now suppresses a new stub when a session log already exists within an **active-session window** (default 2h; override `AGENT_MEMORY_SESSION_WINDOW_HOURS`), nudging you to **enrich that log** instead — detected by the newest session **filename** (immutable + clone-safe; `mtime` is reset by checkout). The old guard only checked for an *untracked* stub, so after the log was committed every later work commit wrote a fresh stub — ~6 near-identical lite logs per session, inflating the decay session-count. A genuinely new session (no log within the window) still gets a stub. From downstream `mercury-composable` feedback |
...
agent-memory/
  ...
  agent-skills/                      ← portable skills: neutral source of truth (vendor adapters gitignored)
    ...
    (the six built-ins ship provenance: agent-memory-builtin and install into every enabled repo)
