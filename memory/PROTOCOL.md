...
3. **Honor multi-agent continuity.** Read `last_session` in `continuity.md`; when it names
...
1. Set `last_session` to today's date, agent name, and log stem. Keep `status` a short
   current-state line, never a version history; history belongs in logs and `UPGRADE.md`.
